...
HTMLFile = codecs.open("dh_user/mail_template.html", 'r', "utf-8").read()
HTMLFileInvite = codecs.open("dh_user/mail_template_invite.html", 'r', "utf-8").read()
...
def send_verify_email (user, link):
    ...
    with smtplib.SMTP_SSL(EMAIL_HOST, EMAIL_PORT) as smtp:
        ...
        for email in set([user.email]):
            msg = EmailMessage()
            msg['Subject'] = "Verify Email Account | Dhoomatech" 
...
def send_invite_email (email, link, groupName):
    ...
    with smtplib.SMTP_SSL(EMAIL_HOST, EMAIL_PORT) as smtp:
        ...
        for email in set([email]):
            msg = EmailMessage()
            msg['Subject'] = "Group Invitation | Dhoomatech" 
            msg['From'] = EMAIL_HOST_USER 
            msg['To'] = email
            msg.set_content(index,subtype='html')
            smtp.send_message(msg)
